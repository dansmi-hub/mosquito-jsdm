...
means <- lapply(1:6, function(x) raster::cellStats(presence_raster[[x]]$Richness_Raster, stat = "mean"))
names(means) <- paste0(resolutions, "km")

# get the frequency of raster values (species present)
freqs <- lapply(1:6, function(x) raster::freq(presence_raster[[x]]$Richness_Raster))
